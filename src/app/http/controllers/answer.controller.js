...
const createAnwser = async (req, res, next) => {
  try {
    ...
    const user = await UserModel.findById(answer.NguoiDung, {
      hoten: 1,
      anhdaidien: 1,
    });
...
const getAnwserByIdQuestion = async (req, res, next) => {
  try {
    const answers = await AnswerModel.find({ CauHoi: req.params.id }).sort({
      thoigian: "desc",
    });
    let list = [];
    for (let i = 0; i < answers.length; i++) {
      const user = await UserModel.findById(answers[i].NguoiDung, {
        hoten: 1,
        anhdaidien: 1,
      });
      list.push({
...
module.exports = { createAnwser, getAnwserByIdQuestion };
